...
class API():
  ...
  def __init__(self, token=None, client_id=None, client_secret=None):
    self.token, self.client_id, self.client_secret = token, client_id, client_secret
    ...
    if req.status_code==200:
      ...
      req = requests.post('https://id.twitch.tv/oauth2/token', data={'client_id':self.client_id,'client_secret':self.client_secret,'grant_type':'client_credentials','scope':None})
      if req.status_code==200:
  ...
  def GetClips(self,broadcaster_id=None,game_id=None,id=None,after=None,before=None,ended_at=None,first=None,started_at=None):
    """OAuth or App Access Token required"""
  ...
  def GetTopGames(self,after=None,before=None,first=None):
    """OAuth or App Access Token required."""
  ...
  def SearchChannels(self,query,first=None,after=None,live_only=None):
  ...
  def GetStreams(self, user_id=None, user_login=None, after=None, before=None, first=None, game_id=None, language=None):
    """OAuth or App Access Token required"""
  ...
  def GetVideos(self,id,user_id,game_id,first,language,period,sort,type,after,before):
    """User OAuth Token or App Access Token"""
...
class GQL():
  ...
  def HomeTrackQuery(self,login):
    return requests.request('POST','https://gql.twitch.tv/gql',data=json.dumps({"operationName": "HomeTrackQuery", "variables": {"channelLogin": login}, "extensions": {"persistedQuery": {"version": 1, "sha256Hash": "ec3cf4f02623f049d9a6c0b02e8c419b55e77c36db785facfade3871d3a86dc0"}}}),headers=self.headers)

  def Comments(self, vodID, content_offset_seconds=None, cursor=None):
    return requests.request('GET',f'https://api.twitch.tv/v5/videos/{vodID}/comments', params={'content_offset_seconds':content_offset_seconds,'cursor':cursor}, headers=self.headers)
